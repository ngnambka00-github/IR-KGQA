...
if __name__ == "__main__":
    ...
    """
            ...
            11
    """
    nodes_data = list(map(lambda x: (x[0], x[2]), all_KG_triples))[0]
    ...
    afm_model_path = "_".join([KG_NAME, "half" if KG_HALF else "full", str(HOPS), "hop", "Fri_May_12_00_03_15_2023"])
    AFM_STORE_PATH = f'../answer_filtering_module/{afm_model_path}/best_afm_model.pt'
    # define class model before loading parameters
    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        if epoch_idx % TEST_INTERVAL == 0:
            ...
            for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len, text_qs in afm_dev_process:
                batch_ranked_topK_entity = afm_model.get_ranked_top_k(batch_questions_index, batch_questions_length,
                                                                      batch_head_entity, max_sent_len,
                                                                      K=USE_TOP_K).tolist()
                ...
                correct_count, wrong_count, accuracy = calcul_accuracy(batch_similarities, final_qs, final_candids,
                                                                       text_qs,
                                                                       batch_answers)
            ...
            for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len, text_qs in afm_test_process:
                ...
                eval_correct_count += correct_count
                eval_wrong_count += wrong_count
            eval_correct_rate = eval_correct_count / (eval_correct_count + eval_wrong_count)  # Combined accuracy on test and dev
