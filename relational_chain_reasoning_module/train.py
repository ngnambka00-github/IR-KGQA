...
if __name__ == "__main__":
    ...
    print(f'Meta Infos ===> KG name:[{KG_NAME}], Gop:[{HOPS}], Is_half:[{KG_HALF}], '
    ...
    print(f"Number of Edges: {KG.number_of_edges()}, Number of Nodes: {KG.number_of_nodes()}")
    ...
    def dataset_trans(batch_ranked_topK_entity, batch_head_entity, batch_answers, text_qs):
        ...
        for idx, head_entity in enumerate(batch_head_entity.tolist()):
            ...
            for candid in candids:
                ...
                # If there is no path for topic and candidate, discard it
                if len(e_path) < 2:
                    continue
                else:
                    relation_chain = []
    ...
    # ==== Calculate the hit@1 accuracy of this batch by (question description, similarity, candidate answer) + (question description, real answer) ====
    def calcul_accuracy(batch_similarities, final_qs, final_candids, text_qs, batch_answers):
        ...
        for i, j, k in zip(batch_similarities, final_qs, final_candids):
            if j not in predicted_qa:
                predicted_qa[j] = (k, i)
            # When the similarity is less than the currently stored similarity, replace
            if i < predicted_qa[j][1]:
                predicted_qa[j] = (k, i)
        correct_count = 0
        wrong_count = 0
        for m, n in zip(text_qs, batch_answers):
            predicted_answer = predicted_qa[m][0]
            if predicted_answer in n:
                correct_count += 1
            else:
                wrong_count += 1
    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len, text_qs in tqdm(afm_dataloader):
        ...
        print(f'Epoch {epoch_idx}-th: Average_Loss: {avg_epoch_loss / len(afm_dataloader) * batch_size}')
        ...
        if epoch_idx % TEST_INTERVAL == 0:
            ...
            for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len, text_qs in tqdm(afm_dev_dataloader):
                batch_ranked_topK_entity = afm_model.get_ranked_top_k(batch_questions_index.cuda(),
                                                                      batch_questions_length.cuda(),
            ...
            for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len, text_qs in tqdm(afm_test_dataloader):
                batch_ranked_topK_entity = afm_model.get_ranked_top_k(batch_questions_index.cuda(),
                                                                      batch_questions_length.cuda(),
                                                                      batch_head_entity.cuda(),
                                                                      max_sent_len, K=USE_TOP_K).indices.tolist()
    ...
    print(f"Final Epoch Has Reached. Stop And Save rcrm-model To {final_model_path}.")
