...
class ContrastiveLoss(torch.nn.Module):
    ...
    loss = (1 - Y)((D(w))^2)/2 + (Y)(max(0, margin - (D(w))))/2 where D(w) = sqrt((output1)^2 - (output2)^2)
    Based on: http://yann.lecun.com/exdb/publis/pdf/hadsell-chopra-lecun-06.pdf
    """

    def __init__(self, margin=None):
        super(ContrastiveLoss, self).__init__()
        self.margin = margin if margin else torch.tensor(2)

    def forward(self, output1, output2, label):
        euclidean_distance = torch.nn.functional.pairwise_distance(output1, output2)
        loss_contrastive = torch.mean((1 - label) * torch.pow(euclidean_distance, 2) + label * torch.pow(torch.clamp(
            self.margin - euclidean_distance, min=0.0), 2))  # clamp夹断用法
        return loss_contrastive

    @staticmethod
    def get_similarity(output1, output2):
        euclidean_distance = torch.nn.functional.pairwise_distance(output1, output2)
